#!/usr/bin/env bash
# fetch-peko-sidecar.sh — copy the freshly-built `peko` binary into
# the desktop's Tauri sidecar location.
#
# Why this exists
# ---------------
# `peko-desktop` is a Tauri app that bundles the `peko` runtime as a
# sidecar (see ADR-043). Tauri requires sidecar binaries to live at
# `src-tauri/binaries/<name>-<rust-target-triple>` so the build script
# can find and bundle them. The two repos (peko-runtime and
# peko-desktop) are independent, so we need an explicit copy step.
#
# Usage
# -----
...
#      <source>/target/<CARGO_PROFILE>/peko[.exe]
# 2. Detect the host target triple via `rustc -vV | grep ^host`.
# 3. Copy to `src-tauri/binaries/peko-<triple>[.exe]` (creating the
#    directory if needed).
# 4. Make the copy executable.
# 5. Verify the copy runs and prints a version line via
#    `peko version`.
#
...
# Source binary.
SOURCE_BIN="$SOURCE/target/$PROFILE/peko"
if [ ! -x "$SOURCE_BIN" ]; then
    echo "❌ peko binary not found at: $SOURCE_BIN" >&2
    echo "   Build it first: cd $SOURCE && cargo build --$PROFILE -p peko" >&2
    exit 1
fi
...
DEST_BIN="$DEST_DIR/peko-$TRIPLE"
# Windows executables carry the .exe suffix; Tauri expects the suffix
# to match the source.
case "$(uname -s)" in
    MINGW*|MSYS*|CYGWIN*) DEST_BIN="$DEST_BIN.exe" ;;
esac
...
# Sanity check — make sure the copy runs and reports a version.
echo "🔍 Verifying $DEST_BIN --version"
if ! "$DEST_BIN" version; then
